# Convex relaxation of the continuous relaxation. 
# Instead of solving the nonconvex best-response function, 
# we compute each school's optimal assignment over the unit
# hypercube, then have them optimize for proximity to that
# within their choice set. 
using JuMP, Ipopt
using Plots
using Random
...
RelaxedEconomy = NamedTuple{(:Y, :p, :u),
                            Tuple{Matrix{Float64},
                            Vector{Float64},
                            Matrix{Float64}}}
...
Relax `E` by computing each school's optimal assignment over the unit hypercube. 
"""
function relax(E::Economy; verbose=false::Bool)::RelaxedEconomy
    (n, m) = size(E.Y)
    ...
    for c in 1:m
        ...
        if E.p' * value.(μ_c) > E.q[c]
            ...
            μ[:, c], stash_obj = value.(μ_c), getobjectivevalue(model) - E.r[c]
...
"""
function assignment(E::Union{Economy, RelaxedEconomy}, X::Matrix{Float64})::Matrix{Float64}
    (n, m) = size(E.Y)
...
"""
function bestresponse(RE::RelaxedEconomy, X::Matrix{Float64}; verbose=false::Bool)::Matrix{Float64}
    (n, m) = size(RE.Y)
    ...
    for c in 1:m
        ...
        # Unlike in discrete case, now it is possible to have
        # complementarity between two students of same type.
        @objective model Max sum((RE.p[s] * (RE.u[s, c] - μ_c[s]))^2 for s in 1:n)
        ...
        for s in 1:n
            ...
            if freedemand[s, c] > eps()
                X_BR[s, c] = min(1, value(μ_c[s]) / freedemand[s, c])
            else
                X_BR[s, c] = X[s, c]
            end
        end
...
"""
function experiment(T=10::Int, n=120::Int, m=8::Int)::NamedTuple
    E = makedata(n, m)
    println("Relaxing E")
    RE = relax(E; verbose=true)
    ...
    for t in 1:T
        ...
        X_BR[:] = bestresponse(RE, X; verbose=false)
        μ_BR[:] = assignment(RE, X_BR)

        # Scale this norm by E.p
        push!(X_gap, sum(abs.((X_BR - X) .* RE.p)))
        push!(μ_gap, sum(abs.((μ_BR - μ) .* RE.p)))
...
# savefig(pl, "./convexplots/convergence.pdf")
# savefig(pr, "./convexplots/demand.pdf")
